...
before Apache forks child processes, so every child inherits it. This is what unlocks OPcache -- no other configuration is required beyond the normal `zend_extension=opcache.so` + `opcache.enable=1` ini settings. Measured effect on a real WordPress install (`wrk -t2 -c100 -d15s`): request latency dropped from ~372ms (cold compile per request) to ~70ms (warm, opcode-cached), and throughput went from ~11-17 req/s to ~187 req/s.
...
- `opcache.jit` was tested (`tracing`, 64M buffer) against a real WordPress install and showed no measurable improvement -- expected, since CMS page rendering is object/array/hook-heavy rather than the tight numeric loops JIT optimizes. Not enabled by default; only worth revisiting for numerically heavy PHP workloads.
